...
11. [制限事項と既知の問題](#制限事項と既知の問題)
12. [今後の改善点](#今後の改善点)
13. [技術的ハイライト](#技術的ハイライト)
...
1. **削除時にValueStoreから実際に削除していない**（`MMyISAM.cpp:143`）
   - インデックスからのみ削除
   - ディスク領域が無駄になる
   - 将来的にガベージコレクションが必要

2. **トランザクション数制限**（最大5つ）
   - `ALLOWED_SAFE_REGISTRY_COUNT = 5`
   - registryIndex 0-4のみ使用可能

3. **並行制御が未実装**
   - ロック機構なし
   - 複数スレッドからの同時書き込みで競合状態の可能性

4. **EXISTS操作のバグ**（`MMyISAM.cpp:167`）
   - `retDataOptr`が初期化されていない
   - 常にfalseを返す可能性

### パフォーマンス的制限

5. **キャッシュサイズが小さい**（3ブロック）
   - 頻繁なページフォルトの可能性
   - 大規模データでのパフォーマンス低下

6. **B-Tree THRESHOLDが小さい**（4）
   - 木が深くなりやすい
   - 検索効率が低下

7. **First Fit方式のメモリ割り当て**
   - O(m)の線形探索
   - フラグメンテーション問題
...
5. **キャッシュサイズの増加**
   - `CACHE_BLOCK_COUNT`を3から128程度に増加
   - 設定ファイルで変更可能に

6. **B-Tree THRESHOLDの調整**
   - `DEFAULT_THRESHOLD`を4から64程度に増加
   - ノードサイズを可変に

7. **メモリアロケータの改善**
   - First Fitから Best Fit または Buddy System へ変更
   - フラグメンテーション対策
...
**5バイトポインタ（optr）**は、このプロジェクトの最も特徴的な技術です。

**利点**:
- **省メモリ**: 通常のポインタ（8バイト）より3バイト削減
- **大規模アドレス空間**: 最大8TBまでサポート
- **ファイル永続化**: ファイル上のアドレスを直接指す
...
**ファイル上に直接配置されるB-Tree**は、データベースの中核技術です。

**特徴**:
- **永続化**: すべてのノードがファイルに直接配置
- **SHA-1キー対応**: 20バイトのハッシュをキーとして使用
- **省メモリ**: optrの使用により、ポインタオーバーヘッドを削減

**技術的挑戦**:
- ノード分割とマージのアルゴリズム
- ファイルI/Oとキャッシュの連携
- THRESHOLD（分岐数）のチューニング

### 3. 簡易トランザクション機構（セーフモード）

**ファイル分離方式**によるトランザクション機構は、独自の設計です。

**特徴**:
- **別ファイルで変更を追跡**: Normal DBとは別ファイルに書き込み
- **commit時にマージ**: ONodeConversionTableでアドレス変換
- **楽観的ロック**: dbStateによる変更検出

**技術的挑戦**:
- ONodeConversionTableの実装
- マージアルゴリズムの最適化
- dbStateの管理

### 4. ブロックチェーン指向設計

**UTXO管理に最適化**された設計は、ブロックチェーン用途を想定しています。

**特徴**:
- **SHA-1キー**: ブロックチェーンで使用されるハッシュをそのまま使用
- **高速なKey-Value操作**: B-Treeによる O(log n) の検索
- **永続化**: 電源断に対応したデータ保存
